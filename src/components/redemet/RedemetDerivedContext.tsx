...
function StormDistanceReading({ layer }: { layer: RedemetStormLayerResponse }) {
  ...
  const distances = (frame && "points" in frame ? frame.points : []).map(distanceFromPelotas).sort((a, b) => a - b);
  ...
  return (
    <div className="redemet-collections__storms">
      <div>
      ...
      <dl aria-label="Raios detectados por distância de Pelotas">
        <div><dt>Até 50 km</dt><dd>{near}</dd></div>
        <div><dt>50 a 150 km</dt><dd>{middle}</dd></div>
        <div><dt>150 a 450 km</dt><dd>{regional}</dd></div>
      </dl>
    </div>
  );
}
  ...
  return (
    <section className="redemet-collections" aria-labelledby="redemet-collections-title">
      ...
      <div className="redemet-collections__list">
        ...
        <CollectionRow icon={Satellite} label="Satélite REDEMET" layer={data.satellite} />
