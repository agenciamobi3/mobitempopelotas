...
Na branch empilhada `work/observatory-weather-inspector`, o Weather Inspector adiciona `subscribeMapClick` ao mesmo runtime Cesium e consulta previsão horária por coordenada somente quando o usuário ativa a ferramenta e clica no globo. A consulta é limitada regionalmente, arredonda coordenadas, repete o gate `observatoryAccess` no servidor e usa Open-Meteo Best Match como `forecast`, nunca como observação. O painel sincroniza o horário modelado ao relógio global e fica deliberadamente desligado durante A/B até existir semântica própria para dois tempos.
...
- Observatório em `/observatorio` com entitlement PRO, gate server-side, robots estrito, shell standalone, Cesium lazy, assets preparados no build, base keyless, terreno com fallback, camadas observacionais, timeline global, cenários compartilháveis, Comparador A/B Swipe e Weather Inspector nas branches empilhadas; `tests/observatory-foundation.test.ts`, `tests/observatory-timeline.test.ts`, `tests/observatory-interface.test.ts`, `tests/observatory-scenario.test.ts`, `tests/observatory-comparison.test.ts` e `tests/observatory-inspector.test.ts` protegem a superfície, incluindo split nativo, um único widget, isolamento A/B, invalidação de renders antigos, exigência de horário antes da entrada, acessibilidade da cortina, bounds do inspetor, gate PRO e separação modelo/observação;
...
O workflow dedicado `.github/workflows/observatory-foundation.yml` aceita `pull_request` para `main`, além de push em `main` e disparo manual. No PR #135, três tentativas atuais encerraram antes de qualquer step: GitHub registrou `runner_id=0`, `runner_name=""` e `steps=[]`. O workflow geral `Qualidade` do mesmo HEAD também apresentou o sintoma de ausência de runner em tentativas anteriores. Esse estado não constitui evidência de falha do código nem aprovação dos contratos, typecheck ou build.

Os PRs #137 e #138 são deliberadamente empilhados sobre a fundação anterior, portanto não são candidatos a promoção direta para `main` enquanto suas bases não estiverem estabilizadas. A validação dirigida deve ocorrer quando houver runner utilizável ou por execução equivalente capaz de alcançar os contratos, typecheck e build; não mascarar indisponibilidade de runner com mudanças fora do escopo.
...
2. Fechar a revisão do PR #137 do Comparador A/B Swipe e validar contratos/typecheck/build quando houver executor funcional; não promover para `main` antes de #135.
3. Revisar e validar o PR #138 do Weather Inspector: confirmar picking Cesium, gate PRO, consulta regional Open-Meteo, sincronização com a timeline e copy que separa previsão modelada de observação; manter draft/empilhado até #137 estar estabilizado.
4. Executar typecheck e os contratos de navegação/transparência assim que houver executor funcional, sem corrigir falhas fora do escopo apenas para produzir verde.
5. Confirmar no domínio canônico o redirect 301 de `/metodologia` e a reabertura `noindex` de `/estacao-embrapa-pelotas`, incluindo leitura real, estado indisponível, atalho no megamenu e permanência fora do sitemap.
6. Observar a estabilidade do `Current_Monitor.htm` em leituras consecutivas antes de discutir retorno da Embrapa ao `Agora`, ao histórico automático, ao sitemap/indexação ou ao footer.
7. Confirmar propagação de `/nivel-do-rio-jaguarao` e `/nivel-do-canal-sao-goncalo`, incluindo HTTP, canonical, Schema, sitemap e links internos.
8. Confirmar o smoke do hub `/nivel-da-lagoa-dos-patos` e das cinco páginas locais.
9. Observar Search Console antes de promover outra estação da Defesa Civil; não expandir automaticamente Turuçu, Cristal, Arroio Grande, Bagé ou Santa Vitória do Palmar.
10. Configurar `MOBI_PORTAL_ADMIN_EMAILS` no runtime e validar Moderação V1 com conta autorizada e contribuição descartável.
11. Fazer E2E autenticado do Widget Builder e do fluxo de contribuição com conta descartável.
12. Executar manualmente o workflow INMET Gmail em modo `check` quando houver runner funcional e comprovar uma mensagem real de previsão de Pelotas antes de reativar Web Push.
13. Confirmar externamente o destino do LabHidroSens; somente com encerramento definitivo comprovado remover ThingsBoard e promover CIEX/FURG a fonte local única.
14. Continuar a recuperação documental das enchentes de 2001 e 2015 pelos caminhos institucionais já identificados.
15. Validar visualmente a família `/tempo-hoje-pelotas`, `/tempo-amanha-pelotas`, `/previsao-7-dias-pelotas`, `/previsao-15-dias-pelotas`, `/chuva-em-pelotas` e `/vento-em-pelotas` em desktop e mobile depois da propagação, conferindo rails, primeira dobra, estados indisponíveis e responsividade sem tratar preview isolado como prova de produção.
16. Validar no preview/domínio o inventário e a hidrografia ANA de `/situacao-hidrologica-pelotas`, o retorno real de `Indice`/`Notas` para `87955000`, a ficha `87955001` e a cronologia 2024–2026 em `/nivel-da-lagoa-dos-patos-laranjal`.
17. Para `87955001`, priorizar a recuperação de ficha de estação/ficha de campo e documentação de RN/nivelamento do sensor. Também buscar documento que ligue explicitamente o sensor ANA anunciado em 27/06/2025 ao código `87955001`; o início cadastral de telemetria em 08/06/2026 e o seletor de ficha observado no HAR estreitam a investigação, mas não substituem essa prova.
18. Validar no preview e no domínio canônico o novo desenho editorial de `/status-dos-dados`, incluindo a linha `Condição do dado`, responsividade das linhas de fonte, histórico aberto e estados live/stale/unavailable, sem tratar preview isolado como prova de produção.
19. Validar no preview e no domínio canônico `/radar-e-satelite-pelotas` depois da propagação: alternância Realçado/IR/Visível, estado noturno do Visível com próxima janela quando recebida, fonte/horário mudando com o produto, contingência INMET sem duplicação, radar REDEMET carregando/sequenciando, STSC distinguindo ausência de coleta de zero real e estabilidade do radar com budget interno de 4,2 s.
20. Manter Service Worker/Web Push suspensos até estabilidade sustentada.
...
- `docs/TEMPO_PELOTAS_OBSERVATORIO_WEATHER_INSPECTOR_2026-09-12.md` — inspeção por ponto, previsão modelada, picking Cesium, bounds regionais e gate PRO;
