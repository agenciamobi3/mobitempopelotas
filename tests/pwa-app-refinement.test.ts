...
test("manifest e metadados moveis permanecem ativos", () => {
  ...
  assert.match(rootRoute, /href: "\/brand\/tempo-pelotas-icon\.png"/);
});
...
test("Web Push permanece fora do root", () => {
  ...
  assert.match(pushManager, /\/brand\/tempo-pelotas-icon\.png/);
  assert.doesNotMatch(rootRoute, /PushNotificationsManager/);
});
...
test("manifest mantém atalhos públicos e o ícone PNG canônico", () => {
  assert.match(manifest, /"name": "Tempo Pelotas"/);
  assert.match(manifest, /"orientation": "any"/);
  assert.match(manifest, /"src": "\/brand\/tempo-pelotas-icon\.png"/);
...
test("offline page continua explicita sobre desatualizacao", () => {
  ...
  assert.match(offlinePage, /src="\/brand\/tempo-pelotas-icon\.png"/);
});
