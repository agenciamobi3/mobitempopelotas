...
const CANONICAL_ICON = "/brand/tempo-pelotas-favicon-2026.png";

test("favicon, shortcut icon e Apple touch icon usam o PNG canônico 2026", () => {
  assert.match(root, /rel: "icon",[\s\S]*href: "\/brand\/tempo-pelotas-favicon-2026\.png"/);
  assert.match(root, /rel: "shortcut icon",[\s\S]*href: "\/brand\/tempo-pelotas-favicon-2026\.png"/);
  assert.match(root, /rel: "apple-touch-icon",[\s\S]*href: "\/brand\/tempo-pelotas-favicon-2026\.png"/);
  assert.doesNotMatch(root, /href: "\/brand\/tempo-pelotas-icon\.svg"/);
});

test("PNG canônico 2026 existe como asset estático e quadrado", () => {
  const png = readFileSync("public/brand/tempo-pelotas-favicon-2026.png");
